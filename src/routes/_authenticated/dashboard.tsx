...
import { fmtMoney, fmtDate, STATUS_LABELS, type RequestStatus } from "@/lib/constants";
import {
  BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, LineChart, Line, CartesianGrid,
} from "recharts";
import { AlertTriangle, Clock, DollarSign, TrendingUp, Users2, FileClock } from "lucide-react";
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [requestsRes, careersRes] = await Promise.all([
        supabase.from("client_requests").select("id,status,category_id,agreed_price,amount_paid,currency,next_follow_up_date,expected_delivery_date,assigned_to,archived_at,project_categories(name_en,name_ar,slug)").is("archived_at", null),
        supabase.from("career_applications").select("id,status", { count: "exact", head: false }),
      ]);
      ...
      for (const r of rows) {
        ...
        const cat = (r as any).project_categories?.name_en ?? "Uncategorized";
        byCat[cat] = (byCat[cat] ?? 0) + 1;
        agreed += Number(r.agreed_price ?? 0);
        paid += Number(r.amount_paid ?? 0);
        if (r.expected_delivery_date && r.expected_delivery_date < today && !["completed","delivered","cancelled","rejected"].includes(r.status)) {
      ...
      return {
        rows,
        byStatus, byCat,
        agreed, paid, remaining: agreed - paid,
        active: rows.filter(r => !["completed","cancelled","rejected"].includes(r.status)).length,
        newLeads: byStatus["new_lead"] ?? 0,
        quotesAwaiting: (byStatus["quote_sent"] ?? 0) + (byStatus["negotiating"] ?? 0),
        approved: byStatus["approved"] ?? 0,
        inProgress: byStatus["in_progress"] ?? 0,
        overdue: rows.filter(r => r.expected_delivery_date && r.expected_delivery_date < today && !["completed","delivered","cancelled","rejected"].includes(r.status)).length,
        followToday: rows.filter(r => r.next_follow_up_date === today).length,
        followWeek: rows.filter(r => r.next_follow_up_date && r.next_follow_up_date >= today && r.next_follow_up_date <= weekAhead).length,
...
function Card({ title, value, icon: Icon, accent }: { title: string; value: string | number; icon: any; accent?: boolean }) {
  return (
    <div className="glass-card p-4 flex items-start justify-between">
      <div>
        <div className="text-xs text-muted-foreground">{title}</div>
        <div className={`text-2xl font-semibold mt-1 ${accent ? "brand-gradient-text" : ""}`}>{value}</div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Overview of client requests, revenue, and follow-ups.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card title={t("careerApps")} value={data.careerCount} icon={Briefcase} />
      </div>
          ...
          <ul className="divide-y divide-border">
            {data.attention.map((a, i) => (
              <li key={a.id + i} className="py-2 flex items-center justify-between text-sm">
                <span>{a.label}</span>
                <a href={`/requests?open=${a.id}`} className="text-primary hover:underline">Open</a>
              </li>
            ))}
          </ul>
        )}
